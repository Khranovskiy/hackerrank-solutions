...
function processData(input) {
    ...
    let queries = parts.slice(2 + N, 2 + N + T)
    queries
        .map(
            query =>
                new RegExp(
...
/*
  ...
  of the word that has our in it, find out the total number of occurrences
  of both its UK and US variants in a given sequence of words.

#Input Format

  First line contains an integer N. N lines follow, each line contains a
  sequence of words (W) separated by a single space.
  Next lines contains an integer T. T testcases follow in a new line. Each line
  contains the UK spelling of a word (W')
...
# Output Format
  ...
  Output T lines and in each line output the number of UK and US version of
...
# Explanation
  In the given 2 lines, we find odour and odor once each. So, the total
  count is 2.
*/
